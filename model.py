...
class Semantic_Mapping(nn.Module):
    ...
    def forward(self, obs, pose_obs, maps_last, poses_last, eve_angle, object_category):
        vr = self.vision_range
        self.init_grid = torch.zeros(
            self.args.num_processes, 1 + len(object_category), vr, vr,
            self.max_height - self.min_height
        ).float().to(self.device)
        self.feat = torch.ones(
            self.args.num_processes, 1 + len(object_category),
            self.screen_h // self.du_scale * self.screen_w // self.du_scale
        ).float().to(self.device)
        
        bs, c, h, w = obs.size()
        # print('=======> obs.shape in forward:', obs.size())
        ...
        # print('========> self.feat.shape:',self.feat.shape)
